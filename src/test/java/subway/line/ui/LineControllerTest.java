...
class LineControllerTest extends AbstractControllerTest {
    ...
    void 정상적으로_생성된다() throws Exception {
        LineResponse givenResult = new LineResponse(1L, "2호선", "초록색",
        ...
        String requestBody = objectMapper.writeValueAsString(new LineCreateRequest("2호선", "초록색", 1L, 2L, 10L));
    ...
    void 정상적으로_전체가_조회된다() throws Exception {
        ...
        var result = new LinesResponse(List.of(
                new LineResponse(1L, "2호선", "초록색",
                        List.of(new InterStationResponse(1L, 2L, 3L, 4L))),
                new LineResponse(2L, "신분당선", "빨간색",
                        List.of(new InterStationResponse(1L, 2L, 3L, 4L)))
    ...
    void 정상적으로_조회된다() throws Exception {
        given(lineQueryService.findById(1L))
                .willReturn(new LineResponse(1L, "2호선", "green", List.of(
                        new InterStationResponse(1L, 2L, 3L, 4L)
                )));
