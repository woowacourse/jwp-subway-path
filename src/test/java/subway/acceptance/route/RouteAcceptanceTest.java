...
class RouteAcceptanceTest extends IntegrationTest {
    ...
    class 거리를_계산할_때 {
        ...
        void 정상적인_요청이_들어오면_거리를_계산한다() {
            final var 첫번째_역_요청_결과 = 역_생성_요청("강남역");
            final var 첫번째_역_ID = 역_생성_결과에서_id_가져오기(첫번째_역_요청_결과);

            final var 두번째_역_요청_결과 = 역_생성_요청("역삼역");
            final var 두번째_역_ID = 역_생성_결과에서_id_가져오기(두번째_역_요청_결과);

            final var 세번째_역_요청_결과 = 역_생성_요청("삼성역");
            final var 세번째_역_ID = 역_생성_결과에서_id_가져오기(세번째_역_요청_결과);

            final var 노선_생성_결과 = 노선_생성_요청("2호선", "초록색", 첫번째_역_ID, 두번째_역_ID, 20L);
            final var 노선_ID = 노선_생성_결과에서_id_가져오기(노선_생성_결과);

            노선에서_역_추가_요청(노선_ID, 첫번째_역_ID, 두번째_역_ID, 세번째_역_ID, 10L);

            final var 역_거리_계산_요청_결과 = 역_거리_계산_요청(첫번째_역_ID, 세번째_역_ID);
