...
import static subway.acceptance.CommonSteps.요청_결과가_반환하는_리소스의_위차가_존재하는지_확인한다;
...
import static subway.acceptance.station.StationSteps.역_제거_요청;
...
class StationAcceptanceTest extends IntegrationTest {
    ...
    class 역을_제거할_때 {
        ...
        void 정상_요청의_경우_역이_정상적으로_제거된다() {
            ...
            final long 역_id = 역_생성_결과.as(StationInfoResponse.class).getId();

            final var 생성된_역_조회_결과 = 역_id_조회_요청(역_id);
            요청_결과의_상태를_검증한다(생성된_역_조회_결과, 정상_요청);

            final var 역_제거_결과 = 역_제거_요청(역_id);
            요청_결과의_상태를_검증한다(역_제거_결과, 정상_요청이지만_반환값_없음);

            final var 제거된_역_조회_결과 = 역_id_조회_요청(역_id);
    ...
    class 역을_조회할_때 {
        ...
        void 없는_역을_조회하려고_시도하면_예외가_발생한다() {
            final var 역_조회_결과 = 역_id_조회_요청(1L);
    ...
    class 역_목록을_조회할_때 {
        ...
        void 정상_요청의_경우_역_목록이_정상적으로_조회된다() {
            final var 생성_전_역_목록_조회_결과 = 역_목록_조회_요청();
            역_목록_크기_비교(생성_전_역_목록_조회_결과, 0);

            역_생성_요청("name");

            final var 역_목록_조회_결과 = 역_목록_조회_요청();
    ...
    class 역_정보를_변경할_때 {
        ...
        void 정상_요청의_경우_역_정보가_정상적으로_변경된다() {
            ...
            final var 역_정보_변경_결과 = 역_정보_변경_요청(역_id, "name2");

            요청_결과의_상태를_검증한다(역_정보_변경_결과, 정상_요청이지만_반환값_없음);

            final var 변경된_역_조회_결과 = 역_id_조회_요청(역_id);
