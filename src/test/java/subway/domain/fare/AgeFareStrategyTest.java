...
class AgeFareStrategyTest {
    ...
    @DisplayName("이전 요금이 0보다 작거나 같으면 요금은 없다.")
    void zero(final int fare) {
        ...
        final double result = ageFareStrategy.calculateFare(fare, passenger, subway);
    ...
    @Test
    @DisplayName("6세 이상, 13세 미만의 어린이인 경우 50% 할인이 적용된다.")
    void kid() {
        final Line lineOfTwo = new Line(2L, "2호선", "초록색", 0);
        lineOfTwo.addSection(GANGNAM, YANGJAE, 8);
        final Subway subway = new Subway(new SubwayJgraphtGraph(List.of(lineOfTwo)));
    ...
    @Test
    @DisplayName("13세 이상, 19세 미만 청소년인 경우 20% 할인이 적용된다.")
    void teen() {
        final Line lineOfTwo = new Line(2L, "2호선", "초록색", 0);
        lineOfTwo.addSection(GANGNAM, YANGJAE, 8);
        final Subway subway = new Subway(new SubwayJgraphtGraph(List.of(lineOfTwo)));
