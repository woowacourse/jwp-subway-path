...
class LinesTest {
    ...
    void 호선을_추가한다() {
        ...
        //when
        final Lines insertedLines = lines.insert(new Line(1L, 일호선, 남색, EMPTY_SECTIONS));

        //then
        assertSoftly(softly -> {
            final List<Line> allLines = insertedLines.getLines();
            softly.assertThat(allLines).hasSize(1);
            softly.assertThat(allLines.get(0)).isEqualTo(new Line(1L, 일호선, 남색, EMPTY_SECTIONS));
        });
    }
    ...
    void 이름이_중복되는_호선은_추가할_수_없다() {
        ...
        //expect
        assertThatThrownBy(() -> lines.insert(new Line(2L, 일호선, 초록색, EMPTY_SECTIONS)))
