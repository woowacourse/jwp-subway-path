...
class LineDomainTest {
    ...
    void 노선_이름의_길이가_1미만_20초과일_경우_예외가_발생한다(final String nameValue) {
        assertThatThrownBy(() -> new LineDomain(nameValue, "초록", new SectionsDomain(Collections.emptyList())))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    void 노선_이름이_null일_경우_예외가_발생한다() {
        assertThatThrownBy(() -> new LineDomain(null, "초록", new SectionsDomain(Collections.emptyList())))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    void 노선_색상이_null일_경우_예외가_발생한다() {
        assertThatThrownBy(() -> new LineDomain("2", null, new SectionsDomain(Collections.emptyList())))
                .isInstanceOf(IllegalArgumentException.class);
    }
