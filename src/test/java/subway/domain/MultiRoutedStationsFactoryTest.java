...
class MultiRoutedStationsFactoryTest {

    @DisplayName("순서가 없는 구간 목록을 노선 별로 전달받아 모든 노선의 역을 연결하는 그래프를 생성한다")
    @Test
    void create() {
        Map<Line, List<Section>> sectionsByLine = Map.of(FIXTURE_LINE_1, List.of(
        ...
        MultiRoutedStations result = MultiRoutedStationsFactory.create(sectionsByLine);

        for (Entry<Line, List<Section>> entry : sectionsByLine.entrySet()) {
            checkResult(entry.getKey(), entry.getValue(), result);
        }
    }
