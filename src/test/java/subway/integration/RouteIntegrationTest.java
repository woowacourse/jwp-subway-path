...
import static subway.integration.builder.LineAssured.request;
...
class RouteIntegrationTest extends ControllerTestConfig {
    ...
    void 출발역에서_도착역까지_도달한_후_다시_되돌아간다고_했을_때_구간이_추가되면_최단_경로가_바뀔_수_있다() {
        ...
        StationAssured
                .request()
                        .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("G", "A", 1L, 1));
    ...
    void 출발역과_도착지가_있을_때_환승을_2번하는_상황에서_최단_경로와_금액을_구한다() {
        ...
        StationAssured
                .request()
                        .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("A", "B", 파랑_노선_식별자값, 1))
                        .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("B", "C", 파랑_노선_식별자값, 2))
                        .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("C", "D", 파랑_노선_식별자값, 3))
                        .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("D", "E", 초록_노선_식별자값, 1))
                        .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("E", "F", 초록_노선_식별자값, 2))
                        .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("F", "G", 주황_노선_식별자값, 10));
    ...
    void 출발역과_도착지가_있을_때_환승을_하지않는_상황에서_최단_경로와_금액을_구한다() {
        ...
        StationAssured
                ...
                .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("D", "E", 초록_노선_식별자값, 1))
                .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("E", "F", 초록_노선_식별자값, 2))
                .역과_구간을_등록한다(상행역_하행역_노선_거리_요청("A", "F", 주황_노선_식별자값, 1));
